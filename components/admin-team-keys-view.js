...
const AdminTeamKeysView = () => {
  ...
  return <section className="mx-auto max-w-[720px] rounded-lg border border-[#b8a18055] bg-[#031827]/90 p-5 shadow-lg">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      <h2 className="flex items-center gap-2 font-[Impact,Arial_Narrow,sans-serif] text-sm tracking-wide"><i className="size-2 rounded-full bg-[#81bd30]" />TEAM REGISTRATION KEYS</h2>
      <button className="flex items-center gap-2 rounded bg-[#639d28] px-4 py-2 text-xs font-bold disabled:opacity-60" type="button" onClick={generate} disabled={generating}><FaPlus />{generating ? "GENERATING..." : "GENERATE NEW KEY"}</button>
    </div>
    ...
    {loading ? <p className="text-xs text-[#8bc832]">Loading keys...</p> : <div className="overflow-x-auto">
      <table className="w-full text-left text-[11px]">
        ...
        <tbody>
          {keys.map((teamKey, index) => <tr className="border-b border-white/10" key={teamKey._id}>
            ...
            <td className="whitespace-nowrap px-2 py-2 font-mono font-medium tracking-wide"><FaKey className="mr-1.5 inline text-[#78b82e]" />{teamKey.key}</td>
            <td className="px-2 py-2"><b className={`rounded px-1.5 py-0.5 ${teamKey.status === "used" ? "bg-[#7a1f1f] text-[#ff9d9d]" : "bg-[#426e12] text-[#c6f45a]"}`}>{teamKey.status === "used" ? "Used" : "Free"}</b></td>
            ...
            <td className="px-2 py-2"><button className="rounded p-1.5 text-[#9faab2] hover:text-white" type="button" onClick={() => copyKey(teamKey.key)} aria-label={`Copy key ${teamKey.key}`}>{copiedKey === teamKey.key ? <FaCheck className="text-[#78b82e]" /> : <FaCopy />}</button></td>
          </tr>)}
