...
const fieldClass = "mt-2 h-11 w-full rounded border border-white/25 bg-[#02121f] px-3 text-sm text-white outline-none focus:border-[#78b82e]";
const roleBadge = { superadmin: "bg-[#39216a] text-[#cb7cff]", admin: "bg-[#426e12] text-[#c6f45a]" };
...
const AdminAddAdminView = () => {
  ...
  return <div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
    <section className="rounded-lg border border-[#b8a18055] bg-[#031827]/90 p-4 shadow-lg">
      <h2 className="mb-4 flex items-center gap-2 font-[Impact,Arial_Narrow,sans-serif] text-sm tracking-wide"><i className="size-2 rounded-full bg-[#81bd30]" />ALL ADMINS</h2>
      {loadingAdmins && <p className="text-xs text-[#8bc832]">Loading admins...</p>}
    ...
    <section className="rounded-lg border border-[#b8a18055] bg-[#031827]/90 p-5 shadow-lg">
      <h2 className="mb-4 flex items-center gap-2 font-[Impact,Arial_Narrow,sans-serif] text-sm tracking-wide"><i className="size-2 rounded-full bg-[#81bd30]" />ADD NEW ADMIN</h2>
      <form onSubmit={submit}>
        ...
        {message && <p className={`mb-4 text-xs ${status === "success" ? "text-[#aee54e]" : "text-red-300"}`}>{message}</p>}
        <button className="h-11 w-full rounded bg-[#639d28] text-sm font-bold disabled:opacity-60" type="submit" disabled={status === "loading"}>{status === "loading" ? "ADDING..." : "ADD ADMIN"}</button>
      </form>
    </section>
  </div>;
};
