...
const AdminChangePasswordView = () => {
  ...
  return <section className="mx-auto max-w-[460px] rounded-lg border border-[#b8a18055] bg-[#031827]/90 p-5 shadow-lg">
    <h2 className="mb-4 flex items-center gap-2 font-[Impact,Arial_Narrow,sans-serif] text-sm tracking-wide"><i className="size-2 rounded-full bg-[#81bd30]" />CHANGE PASSWORD</h2>
    <form onSubmit={submit}>
      ...
      <button className="h-11 w-full rounded bg-[#639d28] text-sm font-bold disabled:opacity-60" type="submit" disabled={status === "loading"}>{status === "loading" ? "UPDATING..." : "UPDATE PASSWORD"}</button>
    </form>
  </section>;
};
