...
const fieldClass = "h-11 flex-1 rounded border border-white/25 bg-[#02121f] px-3 text-sm text-white outline-none focus:border-[#78b82e]";
...
const AdminRegistrationRulesView = () => {
  ...
  return <section className="mx-auto max-w-[720px] rounded-lg border border-[#b8a18055] bg-[#031827]/90 p-5 shadow-lg">
    <h2 className="mb-2 flex items-center gap-2 font-[Impact,Arial_Narrow,sans-serif] text-sm tracking-wide"><i className="size-2 rounded-full bg-[#81bd30]" />REGISTRATION RULES</h2>
    <p className="mb-4 text-xs text-[#9faab2]">Add rules one at a time. Use the arrows to change the order players see them in.</p>
    {loading ? <p className="text-xs text-[#8bc832]">Loading current rules...</p> : <>
      <ol className="mb-4 space-y-2">
        {rules.map((rule, index) => <li className="flex items-center gap-2 rounded border border-white/15 bg-[#02121f]/60 px-3 py-2 text-sm" key={index}>
          <b className="w-5 shrink-0 text-[#78b82e]">{index + 1}.</b>
      ...
      <div className="flex gap-2">
        ...
        <button className="flex items-center gap-2 rounded bg-[#639d28] px-4 text-sm font-bold disabled:opacity-60" type="button" onClick={addRule} disabled={!newRule.trim() || status === "loading"}><FaPlus />ADD NEW</button>
      </div>
      {message && <p className={`mt-3 text-xs ${status === "success" ? "text-[#aee54e]" : "text-red-300"}`}>{message}</p>}
    </>}
  </section>;
};
