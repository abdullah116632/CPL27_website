...
const TeamRegistrationForm = () => {
  ...
  if (status === "success") {
    return <div className="rounded-2xl border border-[#78b82e]/70 bg-[#021827e8] p-5 text-center shadow-2xl backdrop-blur-md min-[680px]:p-8">
      <i className="mx-auto grid size-16 place-items-center rounded-full bg-[#6ca82c] text-3xl not-italic"><FaCircleCheck /></i>
      <h1 className="mt-5 font-[Impact,Arial_Narrow,sans-serif] text-[32px] leading-none tracking-wide min-[680px]:text-[44px]">TEAM <span className="text-[#78b82e]">REGISTERED</span></h1>
      <p className="mt-3 text-sm min-[680px]:text-base">
        <b className="text-[#78b82e]">{registeredTeam?.name}</b> has been successfully registered for the <b className="text-[#78b82e]">CPL - CSE Premier League</b>.
      </p>
      <p className="mt-2 text-xs text-[#c5ccd1]">Our team will review your registration and notify you of the result.</p>
      <button className="mx-auto mt-6 flex cursor-pointer items-center gap-2 rounded-lg border border-white/50 px-8 py-4 text-sm font-bold hover:border-[#78b82e]" type="button" onClick={() => resetForm()}><FaRotateRight />REGISTER ANOTHER TEAM</button>
    </div>;
  }
  ...
  return <form className="rounded-2xl border border-[#c6ae8c]/70 bg-[#021827e8] p-5 shadow-2xl backdrop-blur-md min-[680px]:p-8" onSubmit={submit}>
    ...
    <div className="mt-4 flex flex-wrap items-center gap-4"><button className="cursor-pointer rounded-md border border-dashed border-[#78b82e] px-4 py-2 text-sm font-bold text-[#92c936]" type="button" onClick={addManager}><FaPlus className="mr-2 inline" />ADD MANAGER</button><span className="text-xs text-[#c4ccd1]">ⓘ All manager contact details will be used for official team communication.</span></div>
    <div className="mt-5 flex flex-wrap gap-4"><button className="flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-[linear-gradient(135deg,#82bd36,#57971e)] px-8 py-4 text-sm font-bold hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60" type="submit" disabled={status === "loading"}>{status === "loading" ? <FaSpinner className="animate-spin" /> : <FaUserGroup />}{status === "loading" ? "REGISTERING..." : "REGISTER TEAM"}</button><button className="cursor-pointer rounded-lg border border-white/50 px-8 py-4 text-sm font-bold hover:border-[#78b82e]" type="reset" onClick={(event) => resetForm(event.currentTarget.form)}><FaRotateRight className="mr-2 inline" />RESET</button></div>
