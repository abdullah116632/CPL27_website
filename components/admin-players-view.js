...
const statusBadge = { pending: "bg-[#856406] text-yellow-200", approved: "bg-[#426e12] text-[#c6f45a]", rejected: "bg-[#7a1f1f] text-[#ff9d9d]" };
...
const AdminPlayersView = () => {
  ...
  return <section className="rounded-lg border border-[#b8a18055] bg-[#031827]/90 p-4 shadow-lg">
    <h2 className="mb-4 flex items-center gap-2 font-[Impact,Arial_Narrow,sans-serif] text-sm tracking-wide"><i className="size-2 rounded-full bg-[#81bd30]" />ALL PLAYERS</h2>
    ...
    <p className="mb-2 text-xs text-[#8bc832]">{loading ? "Loading players..." : `Showing ${filtered.length} of ${players.length} players`}</p>
        ...
        <tbody>
          {filtered.map((p, i) => <tr className="cursor-pointer border-b border-white/10 hover:bg-white/10" key={p._id} onClick={() => setSelectedPlayer(p)}>
            ...
            <td className="whitespace-nowrap px-2 py-2">
              {p.status === "pending" && <button className="cursor-pointer rounded bg-[#426e12] px-2 py-1 text-[10px] font-bold text-[#c6f45a] disabled:cursor-not-allowed disabled:opacity-60" type="button" onClick={(event) => { event.stopPropagation(); updatePlayerStatus(p._id, "approved"); }} disabled={updatingId === p._id}>{updatingId === p._id ? "..." : "Approve"}</button>}
